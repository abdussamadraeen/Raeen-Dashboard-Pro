import{state as o,saveSettings as a}from"./state.js";import{dom as e}from"./dom.js";function i(){e.topSitesWidget&&(e.topSitesWidget.innerHTML="",o.settings.topSitesSource==="favorites"?(o.settings.shortcuts||[]).forEach((t,s)=>{const n=document.createElement("a");n.href=t.url,n.className="shortcut",n.style.setProperty("--item-index",s);const r=t.icon||`https://www.google.com/s2/favicons?sz=64&domain_url=${t.url}`;n.innerHTML=`<img src="${r}" alt="${t.name}"><span>${t.name}</span>`,e.topSitesWidget.appendChild(n)}):typeof chrome<"u"&&chrome.topSites&&chrome.topSites.get(t=>{t.slice(0,8).forEach((s,n)=>{const r=document.createElement("a");r.href=s.url,r.className="shortcut",r.style.setProperty("--item-index",n);const m=`https://www.google.com/s2/favicons?sz=64&domain_url=${s.url}`;r.innerHTML=`<img src="${m}" alt="${s.title}"><span>${s.title}</span>`,e.topSitesWidget.appendChild(r)})}),u())}function l(){const t=e.newShortcutName.value.trim(),s=e.newShortcutUrl.value.trim();t&&s&&(o.settings.shortcuts||(o.settings.shortcuts=[]),o.settings.shortcuts.push({name:t,url:s,icon:""}),a(o.settings),i(),e.newShortcutName.value="",e.newShortcutUrl.value="")}function c(t){o.settings.shortcuts.splice(t,1),a(o.settings),i()}function u(){e.shortcutsList&&(e.shortcutsList.innerHTML=(o.settings.shortcuts||[]).map((t,s)=>`
